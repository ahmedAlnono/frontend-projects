let choices = document.querySelectorAll(".chose div")
let score = 0;
let paper = `<div style="outline:15px solid hsl(39, 89%, 49%);" id="paper">
...
let rock  = `<div style="outline:15px solid hsl(230, 89%, 62%);" id="rock">
...
let scissors = `<div class="bottom" style="outline:15px solid hsl(349, 71%, 52%);" id="scissors">
...
let randomChoicesElemnt = [paper , rock , scissors];
let randomChoices = ["paper" , "rock" , "scrssors"];

choices.forEach(choice=>{
    choice.addEventListener("click", function(){
        document.querySelector(".chose").classList.remove("active");
        document.querySelector(".result").classList.add("active");
        let num = Math.floor(Math.random()*3);
        let choice1 = document.createElement("div");
        choice1.innerHTML = choice.innerHTML;
        document.querySelector(".user-choice").appendChild(choice1);
        let div = document.createElement("div")
        div.innerHTML = randomChoicesElemnt[num];
        document.querySelector(".random-choice").appendChild(div);
        let h1 = document.createElement("h1");
        if(choice.id == "paper"){
            if(num == 0){
                h1.innerText = "You Draw"
            }
            else if(num == 1){
        ...
        document.querySelector(".final-result").appendChild(h1);
    })
})

document.querySelector(".play-again").addEventListener("click" , function(){
    document.querySelector(".user-choice").removeChild(document.querySelector(".user-choice").lastChild)
    document.querySelector(".random-choice").removeChild(document.querySelector(".random-choice").lastChild)
    document.querySelector(".final-result").removeChild(document.querySelector(".final-result").lastChild)
    document.querySelector(".result").classList.remove("active");
    document.querySelector(".chose").classList.add("active");
})